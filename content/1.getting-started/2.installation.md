---
title: Installation
description: Get started with VueAirport.
navigation:
  icon: i-lucide-download
seo:
  description: Install VueAirport in your Vue 3 or Nuxt project.
---

VueAirport can be installed in any Vue 3 or Nuxt 3 project using your preferred package manager.

## Package Manager Installation

::steps
### Install the packages

VueAirport is organized as a monorepo with separate packages. Install what you need:

::code-group
```bash [npm]
# Core only
npm install vue-airport

# With base plugins
npm install vue-airport @vue-airport/plugins

# With DevTools
npm install vue-airport vue-airport-devtools
```

```bash [yarn]
# Core only
yarn add vue-airport

# With base plugins
yarn add vue-airport @vue-airport/plugins

# With DevTools
yarn add vue-airport vue-airport-devtools
```

```bash [pnpm]
# Core only
pnpm add vue-airport

# With base plugins
pnpm add vue-airport @vue-airport/plugins
```

```bash [bun]
# Core only
bun add vue-airport

# With base plugins
bun add vue-airport @vue-airport/plugins
```
::

::tip
**Recommended:** Install both core and plugins packages for the full VueAirport experience.
::

### Import the composable

Import `useCheckIn` from the core package:

```vue
<script setup lang="ts">
import { useCheckIn } from 'vue-airport';

// Your component logic
</script>
```

### Import plugins (optional)

If you installed `@vue-airport/plugins`, import plugins separately:

```vue
<script setup lang="ts">
import { useCheckIn } from 'vue-airport';
import { 
  createActiveItemPlugin,
  createValidationPlugin,
  createHistoryPlugin,
  createDebouncePlugin
} from '@vue-airport/plugins';

// Your component logic
</script>
```

### Start building

You're ready to go! Check out the [Examples](/examples) to see VueAirport in action.
::

## Requirements

::warning
VueAirport requires Vue 3.3+ or Nuxt 3+ for proper TypeScript support and composition API features.
::

### Vue 3 Projects

```json [package.json]
{
  "dependencies": {
    "vue": "^3.3.0",
    "vue-airport": "latest",
    "@vue-airport/plugins": "latest"
  }
}
```

### Nuxt 3 Projects

```json [package.json]
{
  "dependencies": {
    "nuxt": "^3.0.0",
    "vue-airport": "latest",
    "@vue-airport/plugins": "latest",
    "vue-airport-devtools": "latest"
  }
}
```

::tip
The `vue-airport-devtools` package provides automatic integration with Vue DevTools in Nuxt projects.
::

## TypeScript Support

VueAirport is written in TypeScript and provides full type definitions out of the box. No additional type packages are needed.

```ts
import { useCheckIn } from 'vue-airport';
import type { DeskCore, CheckInItem } from 'vue-airport';

interface MyData {
  label: string;
  value: number;
}

const { createDesk } = useCheckIn<MyData>();
const { desk } = createDesk('myDesk');

// Fully typed desk and items
const items: CheckInItem<MyData>[] = desk.getAll();
```

## Using Plugins

VueAirport includes built-in plugins in the `@vue-airport/plugins` package:

```ts
import { useCheckIn } from 'vue-airport';
import { 
  createActiveItemPlugin, 
  createHistoryPlugin,
  createValidationPlugin,
  createDebouncePlugin
} from '@vue-airport/plugins';

const { createDesk } = useCheckIn();
const { desk } = createDesk('myDesk', {
  plugins: [
    createActiveItemPlugin(),
    createHistoryPlugin({ maxHistory: 50 }),
  ]
});
```

::note
Check out the [Plugin System](/getting-started/plugins) documentation to learn more about using and creating plugins!
::

## Package Organization

VueAirport is organized as a **monorepo** with separate packages for optimal tree-shaking:

::card-group
  :::card
  ---
  icon: i-lucide-box
  title: vue-airport
  to: https://www.npmjs.com/package/vue-airport
  ---
  Core composable and desk system. Required for all projects.
  :::

  :::card
  ---
  icon: i-lucide-plug
  title: @vue-airport/plugins
  to: https://www.npmjs.com/package/@vue-airport/plugins
  ---
  Built-in plugins (activeItem, validation, debounce, history). Optional.
  :::

  :::card
  ---
  icon: i-lucide-bug
  title: vue-airport-devtools
  to: https://www.npmjs.com/package/vue-airport-devtools
  ---
  Vue DevTools integration. Optional, recommended for debugging.
  :::
::

::tip{icon="i-lucide-info"}
**Tree-shaking benefits:** Only import what you need! The modular structure ensures minimal bundle size for your production builds.
::
