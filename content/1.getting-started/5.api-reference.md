...
| `onBeforeCheckOut` | `(id, data, desk) => boolean \| undefined` | Hook called before check-out, return `false` to cancel | `undefined` |
| `onCheckOut` | `(id, data) => void` | Hook called after successful check-out with the item's data | `undefined` |
...
interface CheckInPlugin<T = any> {
  ...
  onBeforeCheckOut?: (id: string | number, data: T, desk: DeskCore<T>) => boolean | undefined;
  /**
   * Called after an item is successfully checked out.
   * Always receives the item data that was checked out.
   */
  onCheckOut?: (id: string | number, data: T, desk: DeskCore<T>) => void;
